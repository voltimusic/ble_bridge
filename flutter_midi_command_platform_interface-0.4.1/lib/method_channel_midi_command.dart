...
class MethodChannelMidiCommand extends MidiCommandPlatform {
  ...
  Stream<MidiPacket>? get onMidiDataReceived {
    _rxStream ??= _rxChannel.receiveBroadcastStream().map<MidiPacket>((d) {
      ...
      // Ensure all fields have default values to prevent null errors
      var device = MidiDevice(
          dd['id'] ?? "unknown_id",        // Default ID
          dd["name"] ?? "Unknown Device",  // Default Name
          dd["type"] ?? "Unknown Type",    // Default Type
          (dd["connected"]?.toString() ?? "false") == "true" // Ensure boolean conversion
      );

      return MidiPacket(
          Uint8List.fromList(List<int>.from(d["data"] ?? [])),  // Handle missing data
          d["timestamp"] as int? ?? 0,  // Default timestamp
          device
      );
    });
